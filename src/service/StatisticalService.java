...
public class StatisticalService {
	...
	public Map<String, Integer> countBorrowedEquipmentByCategory() throws SQLException {
		...
		String sql = "SELECT c.name AS category, COUNT(*) AS count " + "FROM pay_borrow_equipment pbe "
				+ "JOIN pay_borrow pb ON pbe.payBorrowId = pb.id " + "JOIN equipment e ON pbe.equipmentId = e.id "
				+ "JOIN equipment_category c ON e.equipmentCategoryId = c.id " + "WHERE pb.status = 'BORROWING'"
				+ "GROUP BY c.name";
...
//	    print
	...
	public Map<String, Integer[]> getEquipmentStatusByCategory() throws SQLException {
		...
		String sql = "SELECT c.name AS category, "
				...
				+ "       SUM(CASE WHEN e.status = 2 THEN 1 ELSE 0 END) AS underRepairCount " + "FROM equipment e "
				+ "JOIN equipment_category c ON e.equipmentCategoryId = c.id " + "GROUP BY c.name";
